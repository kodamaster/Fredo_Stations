(function(){
  ...
  // Calculer le centroïde de chaque zone depuis ses polygones
  ...
  const rows=Object.entries(ZONES).map(([id,z])=>({id,nom:z.nom,places:z.places,bikes:counts[id]})).sort((a,b)=>b.bikes.length-a.bikes.length);
